...
**Do NOT** change workflow handling, batch run logic, or any other existing functionality.
  ...
  <verify>
Run `cd frontend && npx tsc --noEmit` to verify TypeScript compilation passes with zero errors. Visually confirm the iteration count shows a dotted underline and hovering displays the breakdown tooltip with per-input value counts and the multiplication formula. Then verify library type restoration: configure a string input with multiple values, save it to a library config, reload the page, load the saved config, and confirm the iteration tooltip shows the correct type label (e.g., "string") rather than "unknown" for the restored string input.
  </verify>
  <done>
